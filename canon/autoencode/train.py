...
def train(
    ...
    checkpoint_path, initial_epoch = find_checkpoint(architecture, n_features)
    ...
    # ------------------------------------------------------------------
    ckpt_dir = os.path.join("checkpoints", architecture.lower(), str(n_features))
    os.makedirs(ckpt_dir, exist_ok=True)

    callbacks = [
        TensorBoard(log_dir=os.path.join("logs", f"{architecture.lower()}_{n_features}"), write_graph=False),
        ModelCheckpoint(
    ...
    img_shape = encoder.input_shape[1:]  # safe with Keras 3
...
def find_checkpoint(architecture: str, n_features: int):
    ckpt_dir = os.path.join("checkpoints", architecture.lower(), str(n_features))
